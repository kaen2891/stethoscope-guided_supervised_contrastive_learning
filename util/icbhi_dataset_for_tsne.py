...
class ICBHIDataset(Dataset):
    def __init__(self, train_flag, transform, args, print_flag=True, mean_std=False):
        ...
        for f in filenames:
            f += '.wav' #-->101_1b1_Al_sc_Meditron.wav
            # get the total number of devices from original dataset (icbhi dataset has 4 stethoscope devices)
            device = f.strip().split('_')[-1].split('.')[0] #-->Meditron / LittC2SE / Litt3200 / AKGC417L --> Meditron
            # get the device information for each wav file            
            self.file_to_device[f.strip().split('.')[0]] = self.device_to_id[device] #-->self.file_to_device[101_1b1_Al_sc_Meditron] = self.device_to_id['Meditron'] --> {'101_1b1_Al_sc_Meditron': 0}
        ...
        annotation_dict = get_annotations(args, data_folder) # --> '226_1b1_Pl_sc_LittC2SE': Start End Crackles Wheezes / 0.036 1.207 0 0 / 1.207 3.550 0 0 / ...
